...
#@jit( parallel=True)
def my_haevy_func(lda, img_data, num_start, pixel_in_row, shared_dict):
...
if __name__ == '__main__':
    ...
    # Файл изображения
    # img_data = spectral.open_image(base_path + r'\flaash2\flaash2.hdr').open_memmap()  # writeable=True
    ...
    # lda_classificate_dir = base_path + "\\flaash2_skip_some_points\\*.txt"
    ...
    lda.add_learn_class(
        LdaLearnClass(name=1, folder=lern_data_dir + 'pochva\\', colour=(139, 69, 19)))
    lda.add_learn_class(
        LdaLearnClass(2, lern_data_dir + 'podlesok\\', (0, 255, 0)))
    lda.add_learn_class(
        LdaLearnClass(3, lern_data_dir + 'suhostoj\\', (143, 188, 143)))
    lda.add_learn_class(
        LdaLearnClass(4, lern_data_dir + 'usihanie\\', (128, 128, 0)))
    lda.add_learn_class(
        LdaLearnClass(5, lern_data_dir + 'zdorov_rast\\', (0, 100, 0)))
    ...
    with Pool(processes=workers_CPU_num, ) as pool:
        ...
        with Manager() as manager:
            ...
            for num_start in range(0, img_data.rows_in_file, strings_per_worker):
                ...
                multiple_results.append(pool.apply_async(my_haevy_func, (
                lda, img_data[num_start:num_end], num_start, pixel_in_row, shared_dict,)))
                #my_haevy_func(lda, img_data[num_start:num_end], num_start, pixel_in_row, shared_dict, )
